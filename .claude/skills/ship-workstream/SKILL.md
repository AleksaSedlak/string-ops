...
The body, written to a temp file first:
...
Open the PRs in wave order so each later body can carry the earlier URLs. After the last PR, edit every earlier PR body (`gh pr edit <n> --body-file`) so all of them list all the URLs.
...
Append to `plans/<workstream>/README.md`:

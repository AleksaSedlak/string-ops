...
7. **Wrap** (`/wrap-workstream`) after the merges. `metrics.sh` writes the row, worktrees are removed, herdr workspaces closed, the plan folder deleted. The row is the only thing that survives.
